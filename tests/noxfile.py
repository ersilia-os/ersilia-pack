...
def start_server_detached(
  session, serve_exe: Path, bundle_path: Path, port: int, logfile: Path, pidfile: Path
):
  cmd = (
...
def ci(session: nox.Session):
  ...
  payload_file = (
    Path(session.posargs[2]) if len(session.posargs) >= 3 else Path("data/payload.json")
  )
  ...
  session.env["PATH"] = (
    str(venv_bin)
    + os.pathsep
    + session.env.get("PATH", "")
    + os.pathsep
    + "/bin:/usr/bin"
  )
  session.env["PIP_REQUIRE_VIRTUALENV"] = "1"
  session.env["VIRTUAL_ENV"] = str(venv_root)
  session.env["PYTHON"] = sys.executable
  session.env["UV_PYTHON"] = sys.executable
  ...
  run_checked(
    ...
    "--depth",
    "1",
    f"https://github.com/ersilia-os/{model_id}.git",
  )
  ...
  while time.time() - start_poll < timeout:
    try:
      with urllib.request.urlopen(
        f"{base_url}/job/status/{job_id}", timeout=10
      ) as resp:
